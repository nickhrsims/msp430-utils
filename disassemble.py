...
#
# Updated for python-3.10 (in-progress) by Nicholas H.R. Sims
...
def addressMode(
    ...
    c = 0
    # source first
    if as_ is not None and src is not None:  # CG2
        if src == 2 and as_ > 1:
            # x = RegisterArgument(core,reg=core.R[src], bytemode=bytemode, am=as)
            x = "#%d" % (None, None, 4, 8)[as_]
        elif src == 3:  # CG3
            # x = RegisterArgument(core,reg=core.R[src], bytemode=bytemode, am=as)
            x = "#%d" % (0, 1, 2, -1)[as_]
        else:
            if as_ == 0:  # register mode
                ...
                raise Exception("addressing mode error")
    # dest
    if ad is not None and dst is not None:
        if ad == 0:
            y = "%s" % regnames[dst]
...
class Instruction:
    ...
    def __init__(self, name, bytemode=0, src=None, dst=None, usedwords=0, cycles=0):
        ...
        if self.dst:
            self.dst = symbol_from_adr(self.dst)
        if self.src:
    ...
    def __str__(self):
        if self.src is not None and self.dst is not None:
            return ("%%-%ds %%s, %%s" % INSN_WIDTH) % (
                "%s%s" % (self.name, (self.bytemode and ".b" or "")),
                self.src,
                self.dst,
            )
        elif self.dst is not None:
            return ("%%-%ds %%s" % INSN_WIDTH) % (
                "%s%s" % (self.name, (self.bytemode and ".b" or "")),
                self.dst,
            )
        else:
    ...
    def jumps(self):
        ...
        return (
            (self.name == "call" or self.dst == "PC")
            and (not self.dst is None)
            and (self.dst[0] == "#")
        )  # XXX relative address mode missing

    def targetAddress(self, address):
        """only valid for instructions that jump; return the target address of the jump"""
        if self.dst and (self.name == "call" or self.name == "br"):
            if self.dst[0] == "#":
...
def disassemble(words):
    ...
    if (opcode & 0xF000) == 0x1000 and (
        ...
        as_ = (opcode >> 4) & 3
        src = opcode & 0xF
        x, y, c = addressMode(bytemode, as_=as_, src=src)
        name, addcyles = singleOperandInstructions[(opcode >> 7) & 0x1F]
        cycles = (
            cycles + c + addcyles
        )  # some functions have additional cycles (push etc)
        ...
        if name == "reti":
            ...
            return Instruction(
                name, bytemode, dst=x, usedwords=usedwords, cycles=cycles
            )
    ...
    # double operand
        ...
        if "%" in y:
            ...
            usedwords = usedwords + 1
        return Instruction(
            name, bytemode, src=x, dst=y, usedwords=usedwords, cycles=cycles
        )
    ...
    # unkown instruction
        ...
        return Instruction(
            "illegal-insn-0x%04x" % opcode, usedwords=usedwords, cycles=cycles
        )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        parser.add_option(
            ...
            "--bin",
            dest="binary",
            help="read data from a binary file",
            metavar="FILE",
        )
        parser.add_option(
            "",
            "--symbols",
            dest="symbols",
            help="read symbol addresses from a text file",
            metavar="FILE",
        )
        parser.add_option(
            "-s",
            "--startadr",
            dest="startadr",
            help="startoffset for binary input",
            type="intautobase",
            default=0,
        )
        ...
        if args:
            ...
            sys.stdout.write(
                "Parameter disassemble: %s  (%d words %d cycles)\n"
                % (insn, words, cycles)
            )
        ...
        if options.binary is not None:
            ...
            for seg in data:
                memwords = [
                    struct.unpack("<H", seg.data[x : x + 2])[0]
                    for x in range(0, len(seg.data), 2)
                ]
                ...
                while offset < len(memwords):
                    ...
                    bytes = " ".join(
                        ["%04x" % x for x in memwords[offset : offset + insn.usedwords]]
                    )
                    instext = str(insn)
